...
QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
    ...
    if (typeof filter.input == 'function') {
        ...
        switch (filter.input) {
            ...
            case 'select':
                h += '<select class="form-control form-control-sm" name="' + name + '"' + (filter.multiple ? ' multiple' : '') + '>';
                if (filter.placeholder) {
...
Utils.changeType = function(value, type, boolAsInt) {
    ...
    case 'date': return this.getTimestamp(value);
    default: return value;
    // @formatter:on
    }
};

Utils.getTimestamp = function(value){
...
QueryBuilder.define('invert', function(options) {
    ...
    this.on('getGroupTemplate.filter', function(h, level) {
        var $h = $(h.value);
        $h.find(Selectors.condition_container).after('<button type="button" class="btn btn-sm btn-default" data-invert="group"><i class="' + options.icon + '"></i> ' + self.translate('invert') + '</button>');
        h.value = $h.prop('outerHTML');
    });
    ...
    if (options.display_rules_button && options.invert_rules) {
        this.on('getRuleTemplate.filter', function(h) {
            var $h = $(h.value);
            $h.find(Selectors.rule_actions).prepend('<button type="button" class="btn btn-sm btn-default" data-invert="rule"><i class="' + options.icon + '"></i> ' + self.translate('invert') + '</button>');
            h.value = $h.prop('outerHTML');
        });
    }
}, {
